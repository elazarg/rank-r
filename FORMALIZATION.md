...
| exact staircase | **Lower-bound direction checked; upper-bound direction open** | `not_schmidtNumberLE_staircaseStateAcrossCut_of_threshold_lt` proves `SN(ρ_p)>k` above each threshold. Proving `SN(ρ_p)≤k` at and below the threshold still depends on the product-seed lemma and twirl preservation; the boundary operator identity itself is checked |
