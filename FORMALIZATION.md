...
| Gram-operator rendering of Proposition 5.3 | **Checked** | `linearGramMap` is the vectorized adjoint composite `L* L`, and `mulVecE_linearGramOperator_eq_linearGramMap` identifies its coordinate action with `linearGramOperator`. `linearGramOperator_posSemidef` proves positivity; `hasCrossedPolarization_iff_isCrossingFixed` is the crossing/reshuffling equivalence; `hasRankOneTraceBound_iff_isBlockPositive_one` and `balancedGramDefect_isBlockPositive` are the level-one input and rank-`r` conclusion in block-positive form |
...
| Corollary 5.4, spectral stability | **Checked** | `rankFactor_balancedSingular` constructs the exact-rank factorization from Mathlib's positive singular spectrum; `exists_balancedSingularFactorization` applies constant-diagonal unitary mixing while preserving the centered Gram norm; `partialTrace_singularValueVariance_le` proves the displayed variance bound |
...
| `def:beta2`, Choi quadratic-form bound | **Checked** | `schmidtOperatorNorm` independently defines the restricted `S(k)` norm; `choiTwoBound_iff_schmidtOperatorNorm_le` identifies its bound predicate with `ChoiTwoBound`, and `schmidtOperatorNorm_choiOf_eq_krausActionNorm` proves the Kraus-action formula for every `k` |
