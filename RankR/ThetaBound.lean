/-
The upper half of `thm:clique`: pair amplification, read in Choi form.
...
This is the sandwich the development previously had only one side of: an upper
bound on a *valid* `λ`, with nothing pinning the optimal one. -/
theorem thetaPositive_graphKraus_iff {E : Finset (U × U)} (hE : ∀ p ∈ E, p.1 < p.2)
  ...
  have := thetaPositive_of_choiTwoBound (A := graphKraus E) (β := 1) zero_le_one
    (choiTwoBound_graphKraus hE) (graphKraus_transpose E) R
  simpa using this
