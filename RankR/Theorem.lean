...
`hFGP` is the double-skew action bound, a hypothesis rather than an `axiom`, so
the dependency is visible in the type. -/
theorem rank_r_partial_trace_of_doubleSkew (hFGP : DoubleSkewBound U V)
...
theorem rank_r_partial_trace_exact_of_doubleSkew (hFGP : DoubleSkewBound U V)
    (C : Matrix (U × V) (U × V) ℂ) :
    hsNormSq (ptraceU C) + hsNormSq (ptraceV C)
...
theorem rank_r_partial_trace_strict_of_doubleSkew (hFGP : DoubleSkewBound U V)
...
theorem rank_eq_of_eq_rank_r_partial_trace_of_doubleSkew (hFGP : DoubleSkewBound U V)
    (C : Matrix (U × V) (U × V) ℂ) (hC : C ≠ 0) (r : ℕ) (hrank : C.rank ≤ r)
