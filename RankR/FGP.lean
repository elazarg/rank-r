...
The route here does not pass through singular values.  Writing `P` for the
orthogonal projection onto the span of the orthonormal pair `x, y`, the quantity
to be bounded is `‖KP‖₂²`, and `KP` is visibly a sum of two rank-one operators —
so the estimate applies to `vec (KP)` directly, with no approximation theorem
and no Ky Fan step.
-/
