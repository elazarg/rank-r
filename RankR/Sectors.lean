...
On the doubled space `(U ⊗ V) ⊗ Q` the partially transposed operator
`ρ₀^{T_{UV}}` splits into a positive part supported on the symmetric
combinations `σ_{ij} = ε_{ij} + ε_{ji}` and a negative part supported on the
antisymmetric combinations `ζ_{ij} = ε_{ij} - ε_{ji}`, where
`ε_{jk} = ē_j ⊗ q_k`.  Both sector operators are recorded here in doubled,
denominator-free form: `Ppos = 2P₊` and `Pneg = 2P₋`, which clears the `1/√2`
normalization of the individual `σ` and `ζ` vectors.
-/
import RankR.Kraus
...
/-- Partial transposition on the `U ⊗ V` factor of `(U ⊗ V) ⊗ Q`, defined
entrywise: the `U ⊗ V` indices are exchanged and the `Q` indices are kept. -/
def ptransposeUV (R : Matrix ((U × V) × Fin s) ((U × V) × Fin s) ℂ) :
    Matrix ((U × V) × Fin s) ((U × V) × Fin s) ℂ :=
  Matrix.of fun x y => R (y.1, x.2) (x.1, y.2)

omit [Fintype U] [Fintype V] in
@[simp] theorem ptransposeUV_apply (R : Matrix ((U × V) × Fin s) ((U × V) × Fin s) ℂ)
    (x y : (U × V) × Fin s) : ptransposeUV R x y = R (y.1, x.2) (x.1, y.2) := rfl

/-- `ε_{jk} = ē_j ⊗ q_k`: the entrywise conjugate of `e j` on `U ⊗ V`, tensored
with the `k`-th standard basis vector of `Q`. -/
def eps (e : Fin s → EuclideanSpace ℂ (U × V)) (j k : Fin s) :
    EuclideanSpace ℂ ((U × V) × Fin s) :=
  WithLp.toLp 2 (fun x => if x.2 = k then conj (e j x.1) else 0)

omit [Fintype U] [Fintype V] in
@[simp] theorem eps_apply (e : Fin s → EuclideanSpace ℂ (U × V)) (j k : Fin s)
    (x : (U × V) × Fin s) : eps e j k x = if x.2 = k then conj (e j x.1) else 0 := rfl

/-- The symmetric combination `σ_{ij} = ε_{ij} + ε_{ji}`. -/
def sigmaV (e : Fin s → EuclideanSpace ℂ (U × V)) (i j : Fin s) :
    EuclideanSpace ℂ ((U × V) × Fin s) :=
  eps e i j + eps e j i

/-- The antisymmetric combination `ζ_{ij} = ε_{ij} - ε_{ji}`. -/
def zetaV (e : Fin s → EuclideanSpace ℂ (U × V)) (i j : Fin s) :
    EuclideanSpace ℂ ((U × V) × Fin s) :=
  eps e i j - eps e j i

omit [Fintype U] [Fintype V] in
@[simp] theorem sigmaV_apply (e : Fin s → EuclideanSpace ℂ (U × V)) (i j : Fin s)
    (x : (U × V) × Fin s) : sigmaV e i j x = eps e i j x + eps e j i x := rfl

omit [Fintype U] [Fintype V] in
@[simp] theorem zetaV_apply (e : Fin s → EuclideanSpace ℂ (U × V)) (i j : Fin s)
    (x : (U × V) × Fin s) : zetaV e i j x = eps e i j x - eps e j i x := rfl

/-- `Ppos = 2P₊`: twice the symmetric sector of the partially transposed
doubled state. -/
noncomputable def Ppos (e : Fin s → EuclideanSpace ℂ (U × V)) :
    Matrix ((U × V) × Fin s) ((U × V) × Fin s) ℂ :=
...
doubled state. -/
noncomputable def Pneg (e : Fin s → EuclideanSpace ℂ (U × V)) :
    Matrix ((U × V) × Fin s) ((U × V) × Fin s) ℂ :=
  ∑ p ∈ Finset.univ.filter (fun p : Fin s × Fin s => p.1 < p.2),
...
theorem qform_Ppos_nonneg (e : Fin s → EuclideanSpace ℂ (U × V))
    (x : EuclideanSpace ℂ ((U × V) × Fin s)) : 0 ≤ (qform (Ppos e) x).re := by
...
used. -/
theorem sum_rankOne_eps_eq (e : Fin s → EuclideanSpace ℂ (U × V)) :
...
theorem rankOne_sigmaV_sub_rankOne_zetaV (e : Fin s → EuclideanSpace ℂ (U × V)) (i j : Fin s) :
...
theorem Ppos_sub_Pneg (e : Fin s → EuclideanSpace ℂ (U × V)) :
