...
finite family `w` transfers to the same bound on its analysis operator.  No square
roots are needed: the synthesis bound is tested on the coefficient vector
`c k = ⟪w k, y⟫` itself, for which the pairing `⟪∑ k, c k • w k, y⟫` equals
