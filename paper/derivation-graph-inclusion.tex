...
Established, and for the first three machine-checked (see below): the exact
constant $\beta_2(\Phi_G)=1$; attainment of the
...
inclusion through level $R$ with failure at $R+1$.
...
content of \cref{prop:layers} are machine-checked in the Lean~4 development
accompanying \texttt{rank-r.tex},
\url{https://github.com/elazarg/rank-r}, in the files \texttt{GraphFamily.lean},
\texttt{Theta.lean} and \texttt{ThetaBound.lean}.
...
Not formalized: \cref{lem:dict}, \cref{lem:witness} and
\cref{thm:inclusion}, together with the free-spectrahedral dictionary they rest
on.  The concrete spaces $\cS_G$ and $\cU_G$, their unital and adjoint
structure, the graph-reduction range, the displayed layer formula, and
invariance of the corrected map are formalized.  The remaining statements
restate \cref{thm:clique} in matrix-convexity language and introduce no new
constant.
