...
\url{https://github.com/elazarg/rank-r}, in
\path{RankR/Companions/Graph/Family.lean},
\path{RankR/Companions/Graph/PairBound.lean},
\path{RankR/Companions/Graph/Layers.lean}, and
\path{RankR/Companions/Graph/Inclusion.lean}.
The rook specialization and the coefficient-space dimensions are checked in
\path{RankR/Companions/Graph/Dimensions.lean} and
\path{RankR/Companions/Graph/Rook.lean}.  The basis-free
positive-cone rendering of
\cref{lem:dict} and the exact threshold \cref{eq:exact} are checked there as
well.
...
\cref{lem:dict}, basis-free positive-cone form
  & \texttt{matrixLevelPositiveCone\_subset\_mapped\_iff}\\
\cref{eq:exact}, basis-free positive-cone form
  & \texttt{graphCliqueLevelInclusion\_iff}\\
inclusion through level $R$ and failure at $R+1$
  & \texttt{graphLevelInclusion\_at\_threshold\_of\_card\_le},
    \texttt{not\_graphCliqueNextLevelInclusion}\\
\cref{eq:levels}
  & \texttt{graphProtectedLevelInclusion\_iff}\\
\cref{cor:rook}, threshold and level separation
  & \texttt{rookGraphLevelInclusion\_iff},
    \texttt{rookGraphProtectedLevelInclusion\_iff}\\
\cref{cor:rook}, coefficient counts
  & \texttt{rookGraphTwoLayerDimension\_sub\_one},
    \texttt{rookAmbientDimension\_sub\_one}\\
\hline
\end{tabular}
\end{center}
...
The inclusion dictionary and \cref{eq:exact} are formalized in a basis-free
positive-cone rendering, rather than by choosing a real basis and reconstructing
the displayed monic pencils.  Inclusion is checked at every finite level of
cardinality at most $R$, and the compressed witness checks failure at the
$(R+1)$-clique level.  A checked padding-and-compression argument propagates
that obstruction to every larger canonical level $\mathbb C^n$, proving the
literal biconditional \cref{eq:levels}.  The concrete spaces $\cS_G$ and
$\cU_G$, their unital and adjoint structure, the graph-reduction range, the
displayed layer formula, invariance of the corrected map, and the positive
compressed clique input with its exact expectation are formalized.  Choosing
the displayed real traceless basis and reconstructing the monic pencils is the
remaining presentation interface.
