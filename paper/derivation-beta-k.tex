...
Fix orthonormal bases of $U$ and $V$, write
...
\begin{proof}
Normalize $\norm K_F=1$ and write $\Sigma=\sum_js_j^2=1$.

\emph{Upper bounds.}  For $k=2$, \cref{eq:pair} at $(a,b)=(1,2)$ gives
$s_1^2+s_2^2\le\frac12$.  For $k=3$, apply \cref{eq:pair} at $(a,b)=(2,3)$:
since $s_2\ge s_3$,
\[
    2s_3^2\le s_2^2+s_3^2\le\tfrac12,
    \qquad\text{so}\qquad
    s_3^2\le\tfrac14 ,
\]
and adding the $k=2$ bound gives $s_1^2+s_2^2+s_3^2\le\frac12+\frac14=\frac34$.
For $k\ge4$ the bound $\sum_{j\le k}s_j^2\le1$ is trivial.
...
\begin{remark}
The pairing estimate $\sum_{j\le k}s_j^2\le\lceil k/2\rceil(s_1^2+s_2^2)$ gives
$\lceil k/2\rceil/2$, which is $k/4$ for even $k$ but $(k+1)/4$ for odd $k$ ---
at $k=3$ it yields only the trivial bound $1$.  What closes the odd case is the
availability of \cref{eq:pair} at the pair $(2,3)$.  That generality was
introduced for a different reason, namely to avoid ordering the singular values
before the final step of the double-skew lemma.
\end{remark}
