...
upper bound is machine-checked.  The dual low-rank projection bound is checked
as well.  Explicit two- and three-term truncations and the full rank-at-most-four
elementary tensor attain the matching coefficient, so the exact
$\beta_k=\min\{1,4/k\}$ statement is checked in the equivalent homogeneous
Choi-form formulation.
\end{remark}
...
$\omega=m^{-1/2}\sum_ae_a\otimes e_a$.  Consequently the invariant line
$\C\omega$ has eigenvalue $\frac{m-1}2$, while its orthogonal complement
splits into eigenvalues $-\frac12$ on
$\operatorname{Sym}^2\ominus\C\omega$ and $+\frac12$ on $\wedge^2$.
For $m\ge3$ the distinguished eigenvalue is the simple largest eigenvalue.
At $m=2$ it ties the $+\frac12$ eigenvalue on $\wedge^2$, so it is not simple,
but the same singular-value calculation gives
\[
    \frac{s_1(K_m)^2}{\norm{K_m}_F^2}
...
The equal-dimension closed form \eqref{eq:Km}, its action on all three spectral
sectors, its Hilbert--Schmidt norm, the attained ratio, and the strict
comparison with $1/4$ are machine-checked in finite coordinates.  The sector
dimension and multiplicity counts and the unequal-dimension embedding sentence
are not separately formalized, and the matching upper bound remains
conjectural.
\end{remark}
