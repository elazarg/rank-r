...
\end{equation}
When $k\ge2$, for a $(k-2)$-subset $L$ let
$N_L:=\#\{j\notin L\}=r-k+2$ and
\begin{equation}
\label{eq:delta}
...
\end{equation}
If moreover $k\ge2$ and every $A_a$ satisfies $A_a^T=A_a$, then
\begin{equation}
\label{eq:A}
    (\Phi\otimes\id)(P_{\mathcal H})
    \preceq
...
$\rank(KP)\le k$.  If $KP=0$, the assertion is immediate.  Otherwise, since
$P$ is a self-adjoint idempotent,
$\ip K{KP}_{\mathrm{HS}}=\norm{KP}_F^2$, so $M:=KP/\norm{KP}_F$ has
...
\begin{lemma}
\label{lem:orth}
The $\eta_I$, $I\in\binom{[r]}k$, are orthonormal.  When $k\ge2$, so are the
$\delta_L$, $L\in\binom{[r]}{k-2}$.
\end{lemma}
...
Assume now $k\ge2$.  For \cref{eq:A} it suffices, since
$\mathcal T\mathcal T^*\succeq0$, to show
...
If $\dim U,\dim V\ge2$, then for
$\Phi=\Lambda_U\otimes\Lambda_V$ one has
$\beta_k=\min\{1,4/k\}$ for every $k\ge2$, so on the complete hypergraph
\[
    (\Phi\otimes\id)(P^{(k)})
    \preceq
...
The normalized Kraus family, its Choi operator, the exact value of $\beta_k$,
and the displayed complete-hypergraph specialization are machine-checked in
...
\path{derivation-higher-arity-probe.py} builds $\eta_I$, $\delta_L$ and
